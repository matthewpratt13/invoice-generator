...
fn it_returns_correct_hours() -> Result<(), Box<dyn Error>> {
    ...
    let friday_schedule = Schedule::new(Some(vec![7, 10]), vec![22, 6], Some(vec![18, 20]), None);
    let friday_weekday = Weekday::Fri;

    let test_schedule: Schedule = daily_schedule.get(&friday_weekday).unwrap().clone();

    assert_eq!(
        test_schedule, friday_schedule,
...
fn it_returns_correct_data() -> Result<(), Box<dyn Error>> {
    ...
    let data: Vec<Data> = setup_data.daily_data[0].get(&date).unwrap().clone();
...
fn it_returns_correct_hours_02() -> Result<(), Box<dyn Error>> {
    ...
    let saturday_schedule = Schedule::new(None, vec![20, 7], None, Some(vec![12, 18]));
    let saturday_weekday = Weekday::Sat;

    let test_schedule: Schedule = daily_schedule.get(&saturday_weekday).unwrap().clone();

    assert_eq!(
        test_schedule, saturday_schedule,
        "returned incorrect schedule – check hours"
    );
...
fn it_returns_correct_hours_03() -> Result<(), Box<dyn Error>> {
    ...
    let sunday_schedule = Schedule::new(None, vec![0, 0], None, None);
    let sunday_weekday = Weekday::Sun;

    let test_schedule: Schedule = daily_schedule.get(&sunday_weekday).unwrap().clone();

    assert_eq!(
        test_schedule, sunday_schedule,
        "returned incorrect schedule – check hours"
    );
...
fn it_returns_correct_hours_04() -> Result<(), Box<dyn Error>> {
    ...
    let monday_schedule = Schedule::new(Some(vec![7, 10]), vec![22, 6], Some(vec![18, 20]), None);
    let monday_weekday = Weekday::Mon;

    let test_schedule: Schedule = daily_schedule.get(&monday_weekday).unwrap().clone();

    assert_eq!(
        test_schedule, monday_schedule,
        "returned incorrect schedule – check hours"
    );
...
fn it_returns_correct_hours_05() -> Result<(), Box<dyn Error>> {
    ...
    let tuesday_schedule = Schedule::new(Some(vec![7, 10]), vec![22, 6], Some(vec![18, 20]), None);
    let tuesday_weekday = Weekday::Tue;

    let test_schedule: Schedule = daily_schedule.get(&tuesday_weekday).unwrap().clone();

    assert_eq!(
        test_schedule, tuesday_schedule,
        "returned incorrect schedule – check hours"
    );
...
fn it_returns_correct_hours_06() -> Result<(), Box<dyn Error>> {
    ...
    let wednesday_schedule =
        Schedule::new(Some(vec![7, 10]), vec![22, 6], Some(vec![18, 20]), None);
    let wednesday_weekday = Weekday::Wed;

    let test_schedule: Schedule = daily_schedule.get(&wednesday_weekday).unwrap().clone();

    assert_eq!(
        test_schedule, wednesday_schedule,
        "returned incorrect schedule – check hours"
    );
...
fn it_returns_correct_hours_07() -> Result<(), Box<dyn Error>> {
    ...
    let thursday_schedule = Schedule::new(Some(vec![7, 10]), vec![22, 6], Some(vec![18, 20]), None);
    let thursday_weekday = Weekday::Thu;

    let test_schedule: Schedule = daily_schedule.get(&thursday_weekday).unwrap().clone();

    assert_eq!(
        test_schedule, thursday_schedule,
        "returned incorrect schedule – check hours"
    );
